"""
Stage 2: MPC 参数调整训练脚本 (Param Adjust) - 进阶避障版
策略网络: DeepGatedActorCriticPolicy (tune_gated_net.py)
环境:     TuningEnv (src/rl_envs/tuning_env.py)

使用方法 (加载基础模型继续训练):
    python scripts/train/train_param_adjust.py --resume ./models/param_adjust/param_adjust_gated_deep_final
"""
import os
import sys
import argparse
...
from src.utils.obstacle_wrapper import StaticObstacleWrapper # [新增] 引入障碍物注入包装器

# ==========================================
# 1. 训练超参数 (集中管理)
# ==========================================
# [修改] 更改实验名称，防止覆盖之前的基础模型
EXP_NAME        = "param_adjust_gated_deep_obs" 
...
# <<< [新增回调] 拦截并在终端/TensorBoard 打印每轮行动参数的平均值
...
def make_env(rank: int, map_obstacle: np.ndarray, input_res: float, seed: int = 0):
    ...
    def _init():
        ...
        # ==========================================
        # [修改] 强制注入随机障碍物，density=0.5 保证路面始终有障碍物
        # ==========================================
...
def train(resume_path: str | None = None):
    print("=" * 60)
    print(f"[Stage 2] MPC 参数调整训练 (进阶避障) ——  {EXP_NAME}")
    print(f"策略网络 : DeepGatedActorCriticPolicy")
    print(f"环境     : TuningEnv + StaticObstacleWrapper (动作空间: 5 维)")
    ...
    callbacks = [
        ...
        RewardComponentsCallback(verbose=0),  # >>> 新增的回调在这里 <<<
        ActionMeanCallback(verbose=0), # <<< [新增] 注册参数均值回调
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 2: MPC 参数调整训练")
    parser.add_argument(
        "--resume",
        type=str,
        default="None",
