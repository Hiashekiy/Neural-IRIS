...
def load_map(map_filename: str) -> np.ndarray:
    ...
    _, map_obstacle = cv2.threshold(maze, 50, 1, cv2.THRESH_BINARY_INV)
...
def make_test_env(map_obstacle: np.ndarray,
    ...
    # 移除 RescaleAction，因为 TuningEnv 内部已经做好了 [-1, 1] 映射
    return env
...
# 3. 实时可视化 (高性能数据更新模式)
# ==========================================
def render_step(render_objs, base_env, action_physical, traj_x, traj_y, velocities, status_text):
...
def run_episode(model, env_wrapped, base_env, venv, render: bool, ep_idx: int, save_video: bool = False, video_path: str = None):
    ...
    while not done:
        ...
        if render:
            ...
            render_step(render_objs, base_env, act_phys,
    ...
    w = env_wrapped
    while hasattr(w, 'env'):
        if isinstance(w, StaticObstacleWrapper):
            # w.print_stats()
            break
        w = w.env
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description="Stage-2 MPC 参数调整测试")
    parser.add_argument('--model',    default='param_adjust_gated_deep_obs_final',
                        help='模型文件名 (不含 .zip)，位于 models/param_adjust/')
    parser.add_argument('--stats',    default='param_adjust_gated_deep_obs_vecnorm.pkl',
                        help='VecNormalize 统计文件，位于 models/param_adjust/')
    parser.add_argument('--map',      default='map1.png',
    ...
    parser.add_argument('--obstacles', action='store_true',
                        help='启用障碍物注入（每次 reset 随机在路径旁撒障碍）')
    parser.add_argument('--density', type=float, default=0.5,
                        help='障碍物采样密度 [0,1]，默认 0.5')
