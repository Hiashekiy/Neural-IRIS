...
def configure_backend(env, backend: str):
    cpp_available = bool(getattr(env.planner, 'use_cpp_neural_iris', False))
    use_cpp = backend == 'cpp' and cpp_available
    env.planner.use_cpp_neural_iris = use_cpp
    if backend == 'cpp' and not cpp_available:
        print("[Backend] requested=cpp actual=python (C++ backend unavailable)")
        return
    actual_backend = 'cpp' if env.planner.use_cpp_neural_iris else 'python'
    print(f"[Backend] requested={backend} actual={actual_backend}")
        ...
        if getattr(base_env, 'use_obstacles', False) and base_env.poly_points_np is not None:
            if step - last_obs_step >= 25:
                ...
                if target_idx > closest_idx + 5 and target_idx < len(path_pts) - 5:
                    ...
                    tx_px, ty_px = int(target_pt[0]/res), int(target_pt[1]/res)
                    ...
                    print(f"[Dynamic Obs] 注入新障碍物！距离 {lookahead_m:.1f} m")
                    last_obs_step = step
...
def main():
    ...
    parser.add_argument('--v-ref', type=float, default=8.0, help='指定基线测试的目标车速 v_ref')
    ...
    print(f"\n开始测试纯 MPC 控制: {args.episodes} episodes  map={args.map}  render={render}  backend={args.backend}")
    ...
    for ep in range(args.episodes):
        ...
        env = make_test_env(map_obstacle, use_obstacles=True, density=args.density)
        configure_backend(env, args.backend)
        base_env = env
