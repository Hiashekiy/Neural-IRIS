...
def render_soft_ellipse(preds, grid_x, grid_y, size=128, temperature=10.0):
    B = preds.shape[0]
    
    center_x = (size / 2.0) + preds[:, 0].view(B, 1, 1)
    center_y = (size / 2.0) + preds[:, 1].view(B, 1, 1)
    
    dx = grid_x[:B] - center_x
    dy = grid_y[:B] - center_y
    
    a = preds[:, 2].view(B, 1, 1) + 1e-4
    b = preds[:, 3].view(B, 1, 1) + 1e-4
    sin_t = preds[:, 4].view(B, 1, 1)
    cos_t = preds[:, 5].view(B, 1, 1)
    ...
    mask = torch.sigmoid(temperature * (1.0 - eq))
    return mask
